...
// dump the symbol table after the binary code
	...
	unlink("/tmp/passone");
        ...
        switch(tok){
          ...
          case MULT: if((tok=yylex()) != INTEG){ // Must use integer as operand
						fprintf(stderr,"Bad operand after MULT is %s\n", yytext);
						exit(1);
					}
					str_6(yytext); // Make it to string
					fprintf(p1,"%d  1111111100%s\n",pc, cstr_6); // Print command that'll need second run through
                   break;
          ...
          case RSHIFT: if((tok=yylex()) != INTEG){
                   ...
                   break;
		  case DIV: fprintf(p1,"%d  1111111110000000\n",pc); // Print command straight up, no second run needed
                   break;
